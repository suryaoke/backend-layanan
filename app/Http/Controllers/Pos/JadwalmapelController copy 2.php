...
class JadwalmapelController extends Controller
{
    ...
    public function JadwalmapelsGenerate(Request $request)
    {
        ...
        $id_hari  = Hari::where('kode_hari', $kode_hari)->first();
        Jadwalmapel::where('id_tahunajar', $tahun)->where('id_hari', $id_hari->id)->delete();
        ...
        if ($kode_hari == "H01" || $kode_hari == "H03" || $kode_hari == "H04") {
            $jumlah = 10;
        } elseif ($kode_hari == "H02") {
            $jumlah = 11;
        } elseif ($kode_hari == "H05") {
            $jumlah = 9;
        }
        ...
        foreach ($pengampu as $pengampus) {
            ...
            if ($totalJpForDayOne[$pengampus->kelas] < $jumlah && $pengampus->mapels->jp <= $jumlah - $totalJpForDayOne[$pengampus->kelas]) {
                ...
                if (!$mapelAlreadyTaught) {
                    ...
                    $hari = Hari::where('kode_hari', $kode_hari)->first(); // Mengambil hari dengan id 1
                    ...
                    $hari = Hari::wherenot('kode_hari', $kode_hari)->inRandomOrder()->first();
                    $params = [
                        'kode_jadwalmapel' => $kode_jadwalmapel,
                        'id_pengampu' => $pengampus->id,
                        'id_hari'   => $hari->id,
        ...
        Jadwalmapel::where('status', 1)->delete();
